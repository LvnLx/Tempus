...
The BPM range users can select from is `1` - `999`, which is among the most flexible for metronome apps, as many set the lower limit to `~30` and the upper limit to `~300`. To accommodate most use cases, users need to be able to make BPM adjustments both quickly and accurately. To enable this single digit incrementing/decrementing, a scroll wheel, and numeric input is supported:
...
Volume adjustment is somewhat supported by different metronome apps, but to allow full flexibility the volume for the beat, accent (note at the beginning of the measures), subdivisions, and app as a whole can be independently set, with the last one inspired by [Gap Click](https://gapclick.app/) developed by [Derek Lee](https://github.com/theextremeprogrammer):
...
A working solution to this problem took a few weeks, tons of brain racking, and some major mental reframing of the problem to arrive at. One of the main functional drivers was enabling real-time user changes. I had mentally associated real-time user changes with changing what was in the audio buffer **immediately**, and while that assessment seemed correct at a glance, one of the key details wasn't obvious:
...
One of the big functional challenges in metronomes is the support for time signatures and beat units, due to the need for the duration of a measure and location for all the notes within the measure to be calculated. Many metronome apps support a list of predefined time signatures (such as `4/4`, `3/4`, `6/8`, `5/4`, `12/8`, ...) and beat units (such as quarter notes, dotted quarter notes, eighth notes, ...) which can drastically simplify the necessary duration and location calculations
...
For those interested in a more technical look at how all of the clip locations and measure length are calculated, please feel free to take a look at the implementation [here](https://github.com/LvnLx/Tempus/blob/main/ios/Runner/Metronome.swift#L207)
...
To enable realtime changes to the metronome's settings while it's being played back a relatively simple pair of locking mechanisms are used to ensure changes aren't made while writing data to the audio buffer, and to ensure clips finish playing once started
...
Most of the app's settings (including metronome state) are stored to the device in a key/value fashion. Since subdivisions and some metadata about the clip sounds available are more easily represented in a structured manner, rather than a custom string format, `JSON` encoders/decoders are used to transmit certain types of data between the Flutter and native layers
...
Support for multiple different measures after another and entire song structures was experimented with, however the audio/metronome technical challenges are much smaller than the UI features needed to make such functionality worth investing time into, particularly for an app that isn't intended to be long-lived and generate substantial revenue

### Testing

Due to the overhead and relatively low value that tests would bring for a metronome app with the current feature set, particularly one not intended to be a long-lived product and generate substantial revenue, the decision was made to not invest time into automated testing. The level of manual testing for the app was relatively low for the current feature set, however if more features such as full song structure support were added this decision would be reevaluated

### Continuous Integration and Deployment

CI/CD pipelines were omitted for similar reasons as outlined in [Testing](#testing), however the addition of Android support would prompt reevaluation on pipelines for both automated testing and deployment
